...
func TestOwnedByGraphTemplate(t *testing.T) {
	...
	graphMgr := templateFieldManager(types.UID("some-graph"), "cm")
...
func TestSimple_RGD_RefusesToAdoptGraphOwnedObject(t *testing.T) {
	...
	require.True(t, hasFieldManager(getConfigMap(t, cl, ns, "graph-owned"), templateFieldManager(graph.GetUID(), "cm")),
