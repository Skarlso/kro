...
## Graph Engine Watch Router Metrics

Emitted by the graph engine's watch coordinator. `graphengine_watch_owner_count`
is deliberately unlabeled because graph keys are high cardinality, so it counts the
graphs the router tracks rather than labeling by key.

| Metric | Type | Description | Stability |
| -------- | ------ | ------------- | ----------- |
| `graphengine_watch_owner_count` | Gauge | Number of graphs currently tracked by the graph-engine watch router | ALPHA |
| `graphengine_watch_request_count` | Gauge | Number of active graph-engine watch requests by GVR and kind (scalar/collection) | ALPHA |
| `graphengine_route_total` | Counter | Total events routed through the graph-engine watch router by GVR | ALPHA |
| `graphengine_route_match_total` | Counter | Total events that matched at least one graph by GVR | ALPHA |
